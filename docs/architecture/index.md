# Architecture & Design Patterns

Concepts and patterns for data pipeline architecture, processing paradigms, and integration.

Browse the topics listed below.

## Topics in this section
*🛠 = includes Products & Tools · 📝 = topic needs content*

- [Batch Processing](batch-processing.md)
- [Data Fabric Architecture](data-fabric-architecture.md)
- [Data Hub Architecture](data-hub-architecture.md)
- [Data Lake Vs Data Warehouse](data-lake-vs-data-warehouse.md)
- [Data Lakehouse](data-lakehouse.md)
- [Data Mesh Architecture](data-mesh-architecture.md)
- [Data Pipeline Architecture](data-pipeline-architecture.md)
- [Event Driven Processing](event-driven-processing.md)
- [Kappa Architecture](kappa-architecture.md)
- [Lambda Architecture](lambda-architecture.md)
- [Medallion Architecture](medallion-architecture.md)
- [Micro Batch Processing](micro-batch-processing.md)
- [Real Time Vs Near Real Time](real-time-vs-near-real-time.md)
- [Stream Processing](stream-processing.md)
