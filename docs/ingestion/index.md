# Data Ingestion

How data is collected and brought into the pipeline: batch, streaming, API-based, and other ingestion methods and semantics.

Browse the topics listed below.

## Topics in this section
*🛠 = includes Products & Tools · 📝 = topic needs content*

- [Api Based Ingestion](api-based-ingestion.md) 🛠
- [At Least Once Semantics](at-least-once-semantics.md)
- [Batch Ingestion](batch-ingestion.md)
- [Change Data Capture](change-data-capture.md)
- [Database Replication](database-replication.md)
- [Exactly Once Semantics](exactly-once-semantics.md)
- [File Based Ingestion](file-based-ingestion.md)
- [Full Load Vs Incremental Load](full-load-vs-incremental-load.md)
- [Idempotent Ingestion](idempotent-ingestion.md)
- [Log Based Ingestion](log-based-ingestion.md)
- [Push Vs Pull Ingestion](push-vs-pull-ingestion.md)
- [Streaming Ingestion](streaming-ingestion.md)
- [Upsert Patterns](upsert-patterns.md)
- [Webhook Ingestion](webhook-ingestion.md) 🛠
