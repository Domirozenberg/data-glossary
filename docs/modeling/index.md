# Data Modeling

Dimensional modeling, star/snowflake schemas, fact and dimension tables, SCDs, and keys.

Browse the topics listed below.

## Topics in this section
*🛠 = includes Products & Tools · 📝 = topic needs content*

- [Anchor Modeling](anchor-modeling.md)
- [Composite Keys](composite-keys.md)
- [Data Aggregation Levels](data-aggregation-levels.md)
- [Data Granularity](data-granularity.md)
- [Data Vault Modeling](data-vault-modeling.md)
- [Dimension Tables](dimension-tables.md)
- [Dimensional Modeling](dimensional-modeling.md)
- [Fact Tables](fact-tables.md)
- [Graph Modeling](graph-modeling.md)
- [Natural Keys](natural-keys.md)
- [Normalized Vs Nonnormalized](normalized-vs-nonnormalized.md)
- [Slowly Changing Dimensions](slowly-changing-dimensions.md)
- [Snowflake Schema](snowflake-schema.md)
- [Star Schema](star-schema.md)
- [Surrogate Keys](surrogate-keys.md)
